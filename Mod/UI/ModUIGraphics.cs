...
public static class ModUIGraphics
{
	...
	public static void Paint(int uiX, int uiY, int uiW, int uiH, mGraphics g)
	{
		...
		ModUI.PaintNativeButton(uiX + 96, uiY + 6, 48, 18, ModGraphics.isFullscreen ? "BẬT" : "TẮT", ModGraphics.isFullscreen, g);
		mFont.tahoma_7_grey.drawString(g, "(F11 / Alt+Enter)", uiX + 150, uiY + 10, mFont.LEFT);

		// 2. Độ Phân Giải Cửa Sổ
		mFont.tahoma_7b_dark.drawString(g, "Độ phân giải cửa sổ:", uiX + 8, uiY + 32, mFont.LEFT);
		int rBtnW = (uiW - 20) / 4;
		for (int r = 0; r < 4; r++)
		{
			int rx = uiX + 6 + r * (rBtnW + 3);
			bool isSelRes = (!ModGraphics.isFullscreen && ModGraphics.resolutionIndex == r);
			ModUI.PaintNativeButton(rx, uiY + 48, rBtnW, 18, ModGraphics.resolutionNames[r], isSelRes, g);
		}

		// 3. Chế độ đồ họa & Khử răng cưa GPU
		mFont.tahoma_7b_dark.drawString(g, "Chất lượng đồ họa (GPU Khử răng cưa):", uiX + 8, uiY + 74, mFont.LEFT);
		for (int q = 0; q < 4; q++)
		{
			int qx = uiX + 6 + q * (rBtnW + 3);
			ModUI.PaintNativeButton(qx, uiY + 90, rBtnW, 18, ModGraphics.graphicsNames[q], ModGraphics.graphicsQuality == q, g);
		}
		...
		{
			mFont.tahoma_7_grey.drawString(g, "* Ultra: GPU khử răng cưa 4x, texture sắc nét.", uiX + 8, descY, mFont.LEFT);
		}
		else if (ModGraphics.graphicsQuality == 1)
		{
			mFont.tahoma_7_grey.drawString(g, "* Medium: Tối ưu mượt, xóa hiệu ứng chiêu & thời tiết.", uiX + 8, descY, mFont.LEFT);
		}
		else if (ModGraphics.graphicsQuality == 2)
		{
			mFont.tahoma_7_grey.drawString(g, "* Low: Xóa background, phông nền sáng siêu nhẹ.", uiX + 8, descY, mFont.LEFT);
		}
	...
	public static bool HandleTap(int px, int py, int uiX, int uiY, int uiW, int uiH)
	{
		...
		{
			int rx = uiX + 6 + r * (rBtnW + 3);
			if (px >= rx && px <= rx + rBtnW && py >= uiY + 48 && py <= uiY + 66)
			{
		...
		{
			int qx = uiX + 6 + q * (rBtnW + 3);
			if (px >= qx && px <= qx + rBtnW && py >= uiY + 90 && py <= uiY + 108)
			{
